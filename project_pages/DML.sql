...
-- Deletes a Customer from database including all information associated with it
...
-- Select used to display store-related info in dropdown
SELECT Stores.location_ID, Stores.location_name FROM Stores;
...
INSERT INTO Orders (customer_ID, order_cost, item_count, pickup, location_ID) VALUES ((SELECT Customer_ID FROM Customers WHERE Customers.first_name = :first_name_from_dropdown_menu AND Customers.last_name = :last_name_from_dropdown_menu), 
    :order_costInput, :item_countInput, 
...
UPDATE Orders SET order_cost = :order_costInput, item_count = :item_countInput, pickup = :pickupInput, location_ID = (SELECT location_ID FROM Stores WHERE Stores.location_name = :location_name_from_dropdown_menu) 
    WHERE order_ID = :order_ID_from_dropdown_menu;

-- Deletes an Order from database including all information associated with it
...
-- Used to display the store-related info in dropdown
SELECT Stores.location_ID, Stores.location_name FROM Stores;
...
UPDATE Items SET item_cost = :item_costInput, item_name = :item_nameInput, location_ID = (SELECT location_ID FROM Stores WHERE Stores.location_name = :location_name_from_dropdown_menu)
    WHERE item_ID = :item_ID_from_dropdown_menu;
...
-- Used to display the customer-related info in dropdown
...
UPDATE Customer_Stores SET customer_ID = (SELECT customer_ID FROM Customers WHERE Customers.customer_ID = :customerID_from_dropdown_menu), location_ID = (SELECT location_ID FROM Stores WHERE Stores.location_ID = :locationID_from_dropdown_menu) 
    WHERE customer_storeID = :customer_storeID_from_dropdown_menu;
...
SELECT Ordered_Items.ordered_itemID, Orders.order_ID, Items.item_name, Items.item_cost, Orders.order_cost FROM Ordered_Items
    LEFT JOIN 
        Orders ON Orders.order_ID = Ordered_Items.order_ID
    LEFT JOIN 
        Items ON Ordered_Items.item_ID = Items.item_ID;

--Used to display the item-related info in dropdown
SELECT Items.item_id, Stores.location_name, Items.item_name FROM Items
    LEFT JOIN 
        Stores ON Items.location_ID = Stores.location_ID;

--Insert into Ordered Items with a new ordered Item
INSERT INTO Ordered_Items (order_ID, item_ID) VALUES (:order_ID_from_dropdown_menu, :item_ID_from_dropdown_menu);
...
UPDATE Ordered_Items SET order_ID = :order_ID_from_dropdown_menu, item_ID = :item_ID_from_dropdown_menu
    WHERE ordered_itemID = :ordered_itemID_from_dropdown_menu;